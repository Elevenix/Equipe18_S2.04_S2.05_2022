...
def get_gdp():
    gdp = pd.read_csv("API_NY.GDP.MKTP.KD_DS2_en_csv_v2_4150850.csv")
...
def get_energy_cons_by_source():
    energy = pd.read_csv("Primary Energy Consumption by source, World, 1980-2016 (in Mtoe).csv")
    ...
    for val in energy_countries:
        energy_country = pd.read_csv(val[0])
